...
class Channel {
  ...
  initializeAudio() {
    this.merger_node = new ChannelMergerNode(audiocontext);
    this.analyzer_node = new AnalyserNode(audiocontext);
    this.merger_node.connect(this.analyzer_node);
    this.analyzer_node.connect(audiocontext.destination);
  }
  ...
  toggle() {
    ...
    this.element.querySelector(".channel_toggle_green").style.backgroundColor = this.enabled?"#5cff5c":"#1a501a";
    if (this.enabled)
      this.draw();
  }
  ...
  draw() {
    ...
    let max = buffer.reduce((prev, current) => {return current > prev ? current : prev;});
